...
class DatabaseManager:
    ...
    def get_top_users_by_bookings(self, year: int = None, month: int = None, limit: int = 3) -> List[Tuple]:
        ...
        print(f"DEBUG: get_top_users_by_bookings called with year={year}, month={month}, limit={limit}")
        ...
        # Отладка: проверим, что есть в таблице stats
        cursor.execute("SELECT COUNT(*) FROM stats")
        total_stats = cursor.fetchone()[0]
        print(f"DEBUG: Всего записей в stats: {total_stats}")
        ...
        for i, result in enumerate(results):
            print(f"  DEBUG: Результат {i+1}: {result}")
    ...
    def get_top_users_by_duration(self, year: int = None, month: int = None, limit: int = 3) -> List[Tuple]:
        ...
        print(f"DEBUG: get_top_users_by_duration called with year={year}, month={month}, limit={limit}")
